...
\begin{enumerate}
  \item \textbf{Beam element interpolation interface.}
    \texttt{extract\_section\_kinematics} currently assumes methods
    (\texttt{sample\_centerline\_translation\_local},
    \texttt{sample\_rotation\_vector\_local}, etc.)  that are not yet
    part of the \texttt{BeamElement} public API.  Adapting the existing
    \texttt{StructuralReductionPolicy} specialisations would close this
    gap.

  \item \textbf{Sub-model solve.}
    The current implementation produces boundary conditions but does not
    solve the continuum sub-models.  Integrating with
    \texttt{NonlinearAnalysis} or \texttt{LinearAnalysis} would enable
    local stress-field resolution.

  \item \textbf{Bidirectional coupling.}
    Homogenising the sub-model response back into the global model
    (e.g.\ updating beam section stiffnesses from continuum results)
    would enable iterative multiscale analysis.

  \item \textbf{PETSc TS state injection.}
    For dynamic transition (Phase~2), injecting velocity and acceleration
    into a \texttt{TS} object requires careful PETSc API usage.  The
    current implementation transfers displacement; full dynamic state
    transfer is an open task.

  \item \textbf{Parallel sub-model creation.}
    Each sub-model in Phase~5 is independent, making the build step
    trivially parallelisable via OpenMP or PETSc's process-level
    distribution.
\end{enumerate}
