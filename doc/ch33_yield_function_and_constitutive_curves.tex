...
Because a data object alone cannot force ParaView to leave the standard render
view and switch into a chart view, the bundle now also emits a small companion
Python script:
\begin{center}
  \texttt{material\_hysteresis\_curves\_paraview\_chart.py}
\end{center}
This script opens the generated \texttt{.vtt} files and constructs an
\texttt{XYChartView} automatically.  In practice, this is the closest thing to
``open the file and see the constitutive curve immediately'' that ParaView
supports while still keeping the actual curve data in a proper tabular format.
...
\begin{itemize}
  ...
  \item \texttt{material\_hysteresis\_curves\_paraview\_chart.py}
\end{itemize}

In ParaView, the intended workflow is now:

\begin{enumerate}
  \item either open one of the \texttt{.vtt} files manually and place it in a
        \emph{Line Chart View},
  \item or run \texttt{material\_hysteresis\_curves\_paraview\_chart.py} from
        ParaView's Python shell to create the chart view automatically;
  \item use \texttt{strain} as the \(x\)-axis array and \texttt{stress} as the
        plotted series;
  \item inspect \texttt{step} or \texttt{path\_parameter} when loading,
        unloading, and reversals need to be highlighted.
\end{enumerate}
