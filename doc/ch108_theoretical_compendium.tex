...
population evolved under belief-space influence (normative intervals
of the elite; situational best) and elitism; a single seeded
\texttt{mt19937\_64} makes runs bit-reproducible. In the solver-tuning
instance the genome is the eight-dimensional LM configuration
(log-scales for $\mu_0$ and the acceptance floor; rounded integers
for stagnation and subdivision genes), the objective replays a
checkpointed protocol window, and the fitness is
$w_1 n_{\text{conv}}/m - w_2\max(0, |V|_{\text{peak}}/V_{\text{env}}
- 1) - w_3\bar\imath$ with $w=(1,1,0.2)$. The same layer is reserved
for control-device optimisation \cite{LaraValenciaEchavarriaValencia2024}.
