...
\begin{itemize}
  ...
  \item the default validation norm is now state-weighted, which is a much
    better convergence proxy than raw Frobenius in the current beam-driven
    setting, but it is still built from the generalized strain coordinates of
    the present transfer operator rather than from a fully systematic dual
    energy norm over the microproblem;
...
\begin{enumerate}
  ...
  \item replace the present generalized-strain proxy weights by a genuinely
    ...
    that validation and FE\textsuperscript{2} convergence use the same
    underlying energetic metric;
