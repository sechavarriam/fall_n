...
condensed operator against the adaptive finite-difference reference using the
global gap
\begin{equation}
  \eta_{\mathbf{D}}
  =
...
\end{equation}
and reports the per-column diagnostics
\begin{equation}
  \eta_j
  =
...
\end{equation}
At present, the automatic accept/reject decision uses the global metric
\(\eta_{\mathbf{D}}\), while \(\max_j \eta_j\) is retained as a visible
diagnostic.  This is intentional: raw section-operator columns mix axial
forces, shear forces, bending moments, and torsional moments, so a naive
columnwise Euclidean norm is informative for auditing but not yet mature enough
to be treated as the sole production acceptance norm.  A weighted,
dimensionally cleaner operator metric remains future work.
...
\begin{description}
  ...
  \item[\texttt{ValidateCondensationAgainstAdaptiveFiniteDifference}] audit
    ...
    finite-difference reference, accept it if the measured global operator gap
    stays below the configured tolerance, otherwise fall back explicitly to the
