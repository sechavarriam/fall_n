...
\begin{itemize}
  ...
  \item \texttt{SectionHomogenizedResponse}: homogenised forces, tangent,
    reference strain, operator provenance, and regularisation diagnostics;
  \item \texttt{CouplingIterationReport}: mode, iteration count, residuals,
    failure count, and macro/micro timing;
...
\begin{itemize}
  \item \texttt{tests/test\_multiscale\_analysis\_api.cpp}:
    ...
    analysis-local coupling start, and accepted FE\textsuperscript{2} commit;
  ...
  \item \texttt{tests/test\_evolver\_advanced.cpp}:
    persistent local-model evolution and homogenisation path.
\end{itemize}
...
\begin{itemize}
  \item the default tangent is still obtained by one-sided perturbation, not by
    a fully consistent algorithmic linearisation;
  ...
  \item \texttt{NonlinearSubModelEvolver} still contains several responsibilities
    internally, even though the external API now isolates the required ones;
...
\begin{enumerate}
  \item replace the forward-difference tangent by a central-difference or
    consistent algorithmic tangent, while retaining the same public response
    contract;
  \item finish the internal split of \texttt{NonlinearSubModelEvolver} into
    solver core, boundary-condition applicator, homogeniser, diagnostics, and
    output services;
