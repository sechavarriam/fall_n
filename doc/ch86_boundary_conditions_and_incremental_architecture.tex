...
1 & + Reconstructed $\boldsymbol{\varepsilon}$, $\boldsymbol{\sigma}$
  ...
  & Proposed; requires \texttt{inject\_initial\_stress} API. \\
2 & + Rebar history ($\boldsymbol{\alpha}_{\text{MP}}$)
  ...
  & Proposed; requires fiber$\leftrightarrow$truss mapping. \\
...
The following concrete steps are proposed:

\begin{enumerate}
  \item \textbf{Extend \texttt{ModelState}:}
    Add an \texttt{InternalVariablesData} variant field to
    \texttt{GaussPointData} that can hold arbitrary per-GP internal
    variable snapshots (using \texttt{std::any} or a type-erased
    buffer).
  \item \textbf{Add \texttt{inject\_gauss\_state()} to the Material
    concept:}  Extend \texttt{InelasticConstitutiveRelation} with a
    method that initialises the internal state from an externally
    provided~$\boldsymbol{\alpha}$.
  \item \textbf{Implement fiber$\,\leftrightarrow\,$truss geometric
    mapping:}  Use the section's fiber coordinates and the sub-model's
    truss node coordinates to establish the bijection
    $\mathcal{M}: \{\text{fibers}\} \to \{\text{trusses}\}$.
  \item \textbf{Implement \texttt{StateTransferProtocol} for
    beam$\,\to\,$continuum:}  A concrete class that performs the
    four-stage transfer described in
