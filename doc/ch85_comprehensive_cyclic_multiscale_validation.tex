...
of regularization.  The Hex8 coarse continuum model (4$\times$3$\times$8)
completes the full protocol with a peak shear of 283.93~kN---substantially
higher than the beam models.  This difference arises from the fully
three-dimensional stress state and the distributed boundary conditions at
the base face, which engage the full confinement capacity of the section.
...
\begin{enumerate}
  ...
  \item The fiber section correctly transfers material-level behaviour
    to the $M$--$\kappa$ plane, with confined concrete increasing
    both strength and ductility.
  \item The MITC Timoshenko beam element converges rapidly;
    $N=4$ typically suffices for engineering accuracy ($<2\%$
    deviation from $N=10$).
  \item The continuum mesh study shows second-order elements (Hex20,
    Hex27) converge faster and more reliably than Hex8 under
    predominantly flexural loading, at higher per-element cost.
  \item One-way FE$^2$ coupling produces an envelope consistent
    with both beam and continuum references.
  \item Two-way FE$^2$ coupling requires tangent stabilisation
    (diagonal clamping or finite-difference fallback) to maintain
