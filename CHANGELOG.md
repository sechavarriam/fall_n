# Historial de desarrollo de fall_n

Este archivo conserva, sin editar, el registro historico de desarrollo que
vivia en `README.md` hasta julio de 2026. Documenta la evolucion de la
arquitectura, el programa de validacion y las decisiones de diseno en orden
aproximadamente cronologico. Para la descripcion actual del proyecto ver
`README.md`.
...
- That localization matters physically: the largest moment/curvature drift sits
  ...
  “global loop problem”; it is a distributed nonlinear response mismatch with a
...
- After correcting that mapping, axial-force closure became essentially exact
  (`max_rel_axial_force_error ≈ 3.9e-7`), which is strong evidence that the
...
- The moment-curvature gap is much smaller than in the structural benchmark,
  ...
  `max_rel_moment_error ≈ 5.53e-1` and `rms_rel_moment_error ≈ 1.04e-1`.
...
- In the elasticized bundle, the same fiber layout closes essentially at
  machine precision:
  `max_rel_moment_error ≈ 4.01e-9`,
  `max_rel_tangent_error ≈ 1.65e-9`,
  `max_rel_axial_force_error ≈ 5.20e-14`.
...
- `steel_monotonic` closes essentially exactly:
  `max_rel_stress_error ≈ 1.54e-6`,
  `max_rel_tangent_error ≈ 3.24e-5`,
  `max_rel_energy_error ≈ 4.39e-7`.
- `steel_cyclic` does **not** close even under the guarded two-level protocol:
  `max_rel_stress_error ≈ 1.29e+1`,
  `max_rel_tangent_error ≈ 9.90e-1`,
  `max_rel_energy_error ≈ 9.93e+1`.
- `concrete_monotonic` remains materially different:
  `max_rel_stress_error ≈ 1.19`,
  `max_rel_tangent_error ≈ 3.28e-1`.
- `concrete_cyclic` also remains open:
  `max_rel_stress_error ≈ 3.26`,
  `max_rel_tangent_error ≈ 1.10e+1`,
  `max_rel_energy_error ≈ 3.04e-1`.
...
- `steel_monotonic`: `fall_n ≈ 8.51e-4 s`, OpenSeesPy `≈ 4.03e-3 s`
- `steel_cyclic`: `fall_n ≈ 1.94e-3 s`, OpenSeesPy `≈ 1.50e-2 s`
- `concrete_monotonic`: `fall_n ≈ 6.46e-4 s`, OpenSeesPy `≈ 3.28e-3 s`
- `concrete_cyclic`: `fall_n ≈ 2.86e-3 s`, OpenSeesPy `≈ 1.82e-2 s`
...
asking the section or the reduced column to “match OpenSees”.
...
- after projecting the OpenSees fiber cloud to the declared `fall_n`
  convention, the two section codes differ only mildly in that fiber’s strain
...
- that makes the current external structural frontier **non-monotone** rather
  than simply “closed through X mm”: `75 mm` remains a pathological OpenSees
...
return “success” while the controlled curvature moved in the wrong direction
...
- `100 mm`: `max_iter = 7`, `avg_iter ≈ 4.85`, `max_bisection_level = 1`;
- `125 mm`: `max_iter = 8`, `avg_iter ≈ 4.77`, `max_bisection_level = 0`;
- `150 mm`: `max_iter = 17`, `avg_iter ≈ 5.08`, `max_bisection_level = 0`;
- `200 mm`: `max_iter = 10`, `avg_iter ≈ 6.08`, `max_bisection_level = 0`;
- `250 mm`: `max_iter = 12`, `avg_iter ≈ 5.92`, `max_bisection_level = 1`;
- `400 mm`: `max_iter = 9`, `avg_iter ≈ 4.85`, `max_bisection_level = 1`.
...
- `basic` keeps the lowest Newton work (`avg_iter ≈ 4.85`, `max_iter = 9`);
- `bt` uses more Newton work (`avg_iter ≈ 6.31`, `max_iter = 19`) but was
  faster in wall time on that single case;
- `l2` lands in between on Newton work (`avg_iter ≈ 5.85`, `max_iter = 11`)
...
`V_max ≈ 25.05 kN`, `M_max ≈ 71.12 kN m`, `max_newton_iterations = 33`, and
...
This is the kind of comparison that matters for the thesis: not “which PETSc
method sounds modern,” but which declared solver policy actually gives a more
...
The result is even more precise than the earlier “localized hotspot” reading:
...
useful because it separates “wrong local event ordering” from “same ordering,
different timing”. The result is encouraging:
...
The next continuum gate is no longer a generic “does the 3D column run?” but a
...
- `Hex20 2x2x10` elasticized closes comfortably in about `9.88–12.17 s`
...
“quadratic embedded rebar is unstable”. The accurate statement is narrower:
...
replaced?”: whether the promoted local slice actually benefits from changing
...
the old “Ko-Bathe is too heavy” suspicion:
