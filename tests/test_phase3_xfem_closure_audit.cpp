...
int main() {
    ...
    f << "  \"phase3_closure_items\": [\n"
         ...
         "    {\"id\": \"3.6\", \"item\": \"FieldSplit/Schur/ASM preconditioning for refined meshes (NZ=5/6/8)\", \"status\": \"scoped_deferred_to_branch\", \"current_evidence\": \"GMRES/ILU rejected (still). PREVIOUS catalog claims 'NZ=5 stalled @17.7mm' AND 'NZ=8 timed out 3600s' are BOTH OUTDATED -- under current driver state with direct LU smoke path, NZ=5 small-strain 200mm completes in 44.3s (peak=0.3471 MN), NZ=6 in 87.6s (peak=0.3214 MN), NZ=8 in 71.9s (peak=0.2770 MN) -- all 0 hard steps, 0 bisections. Peak monotonically decreases as h refines (h-convergence). NZ=5 corotational 144.7s (ratio vs SS 0.9983). FieldSplit/Schur/ASM still scoped for iterative-preconditioner path; LU smoke is sufficient for NZ<=8 today.\"},\n"
