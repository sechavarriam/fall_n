...
- The current weighted norms for condensed-tangent validation and FE2 residuals are physically better than pure Frobenius, but they are still proxies built from generalized strain activity rather than a fully dual energetic norm.
...
- Some heavy validation drivers remain expensive to compile and run.
...
3. Normalize naming and spelling across directories and public types.
4. Clean generated artefacts and keep runtime output out of the repository root.
5. Expand installable public targets beyond `fall_n::multiscale_api`.
6. Clean the remaining LaTeX warnings and align old chapters with the current code.
