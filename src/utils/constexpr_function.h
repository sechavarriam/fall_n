...
#include <bits/refwrap.h>     // ref wrapper, std::_Maybe_unary_or_binary_function
#include <bits/functexcept.h> // __throw_bad_function_call

namespace utils _GLIBCXX_VISIBILITY(default)
{
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<typename _Tp>
    ...
    : std::is_trivially_copyable<_Tp>::type
    { };
  ...
  public:
	...
	using _Local_storage = std::integral_constant<bool, __stored_locally>;
	...
	template<typename _Fn>
	  static void
	  _M_create(_Any_data& __dest, _Fn&& __f, std::true_type)
	  {
	...
	template<typename _Fn>
	  static void
	  _M_create(_Any_data& __dest, _Fn&& __f, std::false_type)
	  {
	...
	static void
	_M_destroy(_Any_data& __victim, std::true_type)
	{
	...
	static void
	_M_destroy(_Any_data& __victim, std::false_type)
	{
...
#if __cpp_rtti
	      __dest._M_access<const std::type_info*>() = &typeid(_Functor);
#else
	      __dest._M_access<const std::type_info*>() = nullptr;
#endif
	      break;
	...
	template<typename _Fn>
	  ...
	  noexcept(std::__and_<_Local_storage,
			  std::is_nothrow_constructible<_Functor, _Fn>>::value)
	  {
  ...
  template<typename _Res, typename _Functor, typename... _ArgTypes>
	...
	{
	  return std::__and_<typename _Base::_Local_storage,
			std::is_nothrow_constructible<_Functor, _Fn>>::value;
	}
    };
  ...
  // would be ill-formed.
  template<typename _Signature, typename _Functor,
	   bool __valid = std::is_object<_Functor>::value>
    struct _Target_handler
    : _Function_handler<_Signature, typename std::remove_cv<_Functor>::type>
    { };
  ...
  template<typename _Res, typename... _ArgTypes>
    ...
    : public std::_Maybe_unary_or_binary_function<_Res, _ArgTypes...>,
      private _Function_base
    {
      ...
      template<typename _Func,
	       bool _Self = std::is_same<std::__remove_cvref_t<_Func>, function>::value>
	using _Decay_t
	  = typename std::__enable_if_t<!_Self, std::decay<_Func>>::type;

      template<typename _Func,
	       typename _DFunc = _Decay_t<_Func>,
	       typename _Res2 = std::__invoke_result<_DFunc&, _ArgTypes...>>
	struct _Callable
	: std::__is_invocable_impl<_Res2, _Res>::type
	{ };

      template<typename _Cond, typename _Tp = void>
	using _Requires = std::__enable_if_t<_Cond::value, _Tp>;
	...
	if (_M_manager)
	  {
	    ...
	    if (auto __ti =  __typeinfo_result._M_access<const std::type_info*>())
	      return *__ti;
	  }
	return typeid(void);
      }
	...
	{
	  if _GLIBCXX17_CONSTEXPR (std::is_object<_Functor>::value)
	    {
  ...
  namespace __detail::__variant
  {
    ...
    template<typename _Signature>
      struct _Never_valueless_alt<function<_Signature>>
      : std::true_type
      { };
  }  // namespace __detail::__variant
#endif // C++17
