...
public:
    ...
    void write_with_local_states(const std::string& filename,
        ...
        set_block(multiblock, 0, build_axis_block(accessor), "axis");
        set_block(multiblock, 1, build_surface_block(accessor), "surface");
        set_block(multiblock, 2, build_sections_block(accessor), "sections");
        set_block(multiblock, 3, build_material_sites_block(accessor), "material_sites");
        set_block(multiblock, 4, build_fibers_block(accessor), "fibers");
...
private:
    ...
    vtkSmartPointer<vtkPolyData> build_beam_surface_block(StateAccessor&& accessor) const {
        ...
        const std::size_t ring_size = BeamProfileT::num_boundary_points();
        ...
        for (const auto& element : model_->elements()) {
            ...
            for (const auto& [xi, sec_snapshot] : stations) {
                ...
                for (std::size_t i = 0; i < ring_size; ++i) {
                    const auto yz = beam_profile_.boundary_point(i);
