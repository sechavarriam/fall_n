...
canonical_reduced_rc_column_evidence_closure_table() noexcept
{
    ...
    return std::to_array({
        ...
        make_reduced_rc_column_evidence_closure_row(
            ...
            "moment_curvature_base.csv exported from the active fixed-side station",
            "base-side section observable exists, but today it is only a runtime contract and not yet an independently benchmarked section response",
            "build an independent section-level moment-curvature baseline with the same fiber section and axial preload, then compare envelopes and tangents",
            "moment_curvature_closure.csv + section-baseline comparison tables + quadrature interpretation note",
            "agreement of base-side moment-curvature envelope and tangent evolution against the independent section baseline",
        ...
        make_reduced_rc_column_evidence_closure_row(
            ...
            "runtime family across TimoshenkoBeamN<N> for N=2..10",
            "the refinement family exists but has not yet been reduced to an explicit convergence table",
            "postprocess the N=2..10 family into a convergence study over stiffness, peak strength, dissipation, and loop distance",
            "node_refinement_summary.csv + convergence tables + stabilization interpretation",
            "stable refinement trend with bounded response drift as N increases",
        ...
        make_reduced_rc_column_evidence_closure_row(
            ...
            "beam-axis quadrature is an explicit model axis and runtime slices exist for Gauss/Lobatto/Radau",
            "quadrature families are implemented and exercised, but their response spread has not yet been quantified as a controlled sensitivity study",
            "postprocess the quadrature family sweep into response-spread tables for global loops and base-side moment-curvature",
            "quadrature_sensitivity_summary.csv + family-spread tables + station-placement interpretation",
            "bounded family-to-family spread with explicit interpretation of section-station placement effects",
