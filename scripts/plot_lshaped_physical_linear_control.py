...
def main() -> int:
    ...
    ax.set_title("Edificio en L de 16 pisos, MYG004 físico escala=1: respuesta de control lineal")

    orbit = axes[1]
    orbit.plot(ux, uy, color="#333333", lw=0.75)
    orbit.plot([ux[0]], [uy[0]], "o", color="green", ms=4, label="inicio")
    orbit.plot([ux[-1]], [uy[-1]], "s", color="red", ms=4, label="fin")
