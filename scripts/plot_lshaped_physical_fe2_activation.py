...
def main() -> int:
    ...
    ax.set_ylabel("Desplazamiento de techo (m)")
    ax.set_title("Alarma lineal por fluencia del acero, escala física=1")
    ...
    ax.axvline(alarm_t, color="k", ls="--", lw=0.8)
    ax.set_ylabel("Índice de demanda de fluencia del acero")
    ax.set_title("Compuerta de activación")
    ax.grid(True, alpha=0.35, lw=0.3)

    ax = axes[1, 0]
    ax.plot(t_cracks, total_cracks, "o-", lw=0.8, ms=3, color="#1f77b4", label="fisuras")
    ax.set_xlabel("Tiempo (s)")
    ax.set_ylabel("Fisuras totales")
    ax2 = ax.twinx()
    ax2.plot(t_cracks, max_opening, "s-", lw=0.8, ms=3, color="#d62728", label="apertura máxima")
    ax2.set_ylabel("Apertura máxima (mm)")
    ax.set_title("Respuesta local XFEM unidireccional")
    ax.grid(True, alpha=0.35, lw=0.3)
    ...
    ax.invert_yaxis()
    ax.set_xlabel("Puntaje de sitio macro-inferido")
    ax.set_title("Sitios locales activados")
    ax.grid(True, axis="x", alpha=0.35, lw=0.3)

    fig.suptitle("Prueba de humo de activación FE2/XFEM, escala física=1, desde alarma de fluencia del acero (MYG004)", fontsize=12)
