...
def main() -> int:
    ...
    ax = axes[0, 0]
    ax.plot(t_ref, u_ref, color="#888888", lw=0.9, label="solo global anterior")
    ax.plot(t_env, u_env, color="#1f77b4", lw=0.8, label="FE2 unidireccional XFEM")
    ax.set_title("Respuesta de techo")
    ax.set_xlabel("tiempo físico (s)")
    ...
    ax.set_title("Evolución local XFEM")
    ax.set_xlabel("tiempo físico (s)")
    ...
    ax = axes[1, 1]
    ax.bar(cell_id, pos_fraction, color=colors, width=0.8, alpha=0.75, label="fracción positiva")
    ax2 = ax.twinx()
    ax2.plot(cell_id, interface_area, color="#111111", lw=0.9, marker=".", ms=3, label="área de interfaz")
    ax.set_ylim(-0.03, 1.03)
    ax.set_title("Auditoría de celdas enriquecidas")
    ax.set_xlabel("elemento local")
    ax.set_ylabel("fracción de volumen positivo")
    ax2.set_ylabel(r"área de interfaz (m$^2$)")
    ax.grid(True, alpha=0.25, lw=0.3)

    fig.suptitle("FE2 unidireccional XFEM macro-inferido, 10 s, auditoría de celdas", fontsize=12)
